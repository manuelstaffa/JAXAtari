...
def asymmetric_ellipse_exponential_distance(
    pts, center, left_radius, right_radius, front_radius, back_radius, alpha=ALPHA
):
...
def main():
    ...
    plt.title(
        f"Asymmetric ellipse exponential heatmap — left={LEFT_RADIUS}, right={RIGHT_RADIUS}, front={FRONT_RADIUS}, back={BACK_RADIUS}, alpha={ALPHA}"
    )
    plt.xlabel("x")
    plt.ylabel("y")
