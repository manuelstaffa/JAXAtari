#!/usr/bin/env python3
"""
Heatmap visualizer for the linear asymmetric-ellipse distance.

Values are normalized to the range [0, 1], where 1 is at the center and
0 is at or beyond the ellipse boundary.

Run: python3 rewards/freeway/visualize_asymmetric_ellipse_linear_heatmap.py
"""
...
GRID_RES = 600
SAVE_PNG = False
OUTPUT_FILE = "asymmetric_ellipse_linear_heatmap.png"


def asymmetric_ellipse_linear_distance(
    ...
    return np.clip(1.0 - np.clip(ellipse_distance, 0.0, 1.0), 0.0, 1.0)
...
def main():
    ...
    values = asymmetric_ellipse_linear_distance(
        pts, CENTER, HORIZONTAL_RADIUS, FRONT_RADIUS, BACK_RADIUS
    )
    ...
    plt.colorbar(im, label="linear distance value")
    plt.contour(
        xx,
        yy,
        values,
    ...
    plt.title(
        f"Asymmetric ellipse linear heatmap — horiz={HORIZONTAL_RADIUS}, front={FRONT_RADIUS}, back={BACK_RADIUS}"
    )
    plt.xlabel("x")
    plt.ylabel("y")
